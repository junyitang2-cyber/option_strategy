...
function phase7aMemo(config) {
  return {
    ...
    profitLogic: {
      makesMoneyFrom: config.makesMoneyFrom,
      losesMoneyFrom: config.losesMoneyFrom,
      bestMarketCondition: config.bestMarketCondition,
      worstScenario: config.worstScenario,
    },
    clientPerspective: {
      whyClientDoes: config.whyClientDoes,
      clientType: config.clientType,
      suitability: config.suitability,
    },
    dealerPerspective: {
      whenDealerSells: config.whenDealerSells,
      exposure: config.dealerExposure,
      hedging: config.hedging,
      profitSource: config.profitSource,
    },
    commonMistakes: config.commonMistakes,
    interviewQuestions: [
      {
        q: `${config.name} 的核心 trade-off 是什么？`,
        a: config.tradeoffAnswer,
      },
      {
        q: `Dealer 如何看 ${config.name} 的 risk？`,
        a: config.dealerAnswer,
      },
      {
        q: `${config.name} 最常见的错误表达是什么？`,
        a: config.mistakeAnswer,
      },
    ],
  };
}
...
const PROFESSIONAL_CONTENT = {
  ...
  // Guts - 胆识策略
  "bull-put-spread": phase7aMemo({
    name: "Bull Put Spread",
    directional: "+Delta：short put 是主要风险腿，long lower put 限制尾部亏损。",
    volatility: "-Vega：IV 回落有利，IV 上升会推高 short put spread 的回补成本。",
    time: "+Theta：只要 spot 维持在 short put strike 上方，时间流逝有利。",
    convexity: "-Gamma：下跌接近 short put strike 时 delta 会快速变负。",
    makesMoneyFrom: "收取 credit，并希望标的维持在 short put strike 上方；也可以赚 IV 回落和时间价值衰减。",
    losesMoneyFrom: "标的跌破 short put strike，spread 走向 max loss；越接近 expiry，short strike 附近的 Gamma 越集中。",
    bestMarketCondition: "温和看涨或区间偏强、IV 偏高、愿意用 defined max loss 表达 short put risk。",
    worstScenario: "快速下跌穿过两条 put strikes，bid-ask 变宽，想止损时接近 max loss。",
    whyClientDoes: ["想收 premium 但不愿承担 naked short put 的开放尾部风险。", "愿意在某个下方价位承担有限亏损。", "用 defined-risk credit spread 表达温和看涨或卖 vol 观点。"],
    clientType: "Income-oriented 或 defined-risk bullish client。",
    suitability: "适合能理解 max loss、margin、assignment 和 short strike 附近 Gamma 风险的客户。",
    whenDealerSells: "客户买入 Bull Put Spread exposure 时，dealer 通常站在相反的 spread book 一侧。",
    dealerExposure: "Dealer 需要管理 short/long put strikes 的 net Delta、short/long Gamma 差异、put skew 和 assignment risk。",
    hedging: ["用 underlying 调整 spread 的 net Delta。", "spot 接近 short put strike 时提高 hedge 频率。", "监控 early assignment、dividend、borrow 和 put skew 变化。"],
    profitSource: "Bid-ask spread、skew pricing、客户为 defined-risk short put exposure 支付的 execution spread。",
    commonMistakes: ["错误表达: Bull Put Spread 是低风险收租。正确说法是 max loss 有定义，但亏损概率和回撤速度仍要 stress。", "错误表达: 只要到期高于 short put 就一定舒服。正确说法是 mark-to-market 和 liquidity 会先影响风险管理。", "错误表达: long put wing 解决所有问题。正确说法是 wing 限制尾部，但不能消除 gap、assignment 和 exit cost。"],
    tradeoffAnswer: "Bull Put Spread 用放弃一部分 premium 换 defined max loss。它比 naked short put 更适合风险预算明确的账户，但仍是 short downside Gamma 和 short Vega。",
    dealerAnswer: "Dealer 会把它看成两个 put strikes 的风险桶，而不是单一方向仓位；spot 接近 short put 时 hedge 需求和 assignment 监控都会上升。",
    mistakeAnswer: "最常见错误是把 defined risk 说成 low risk。专业表达应该先讲 max loss、break-even、short strike Gamma、assignment 和 liquidity。",
  }),

  "long-put-butterfly": phase7aMemo({
    name: "Long Put Butterfly",
    directional: "近似中性到轻微看跌：收益集中在 body strike 附近。",
    volatility: "-Vega 为主：进入目标区间后 IV 回落通常有利。",
    time: "+Theta near body，-Theta away from body：时间价值对 payoff 的影响取决于 spot 位置。",
    convexity: "分段 Gamma：body 附近 short convexity，两侧 long wings 限制风险。",
    makesMoneyFrom: "标的到期接近中间 put strike，两个 short puts 衰减最多，而 wings 保持定义风险。",
    losesMoneyFrom: "标的远离目标区间，debit 可能亏掉；过早入场也会被路径和 bid-ask 消耗。",
    bestMarketCondition: "预期标的会收敛到某个下方目标价，且不需要开放式下行收益。",
    worstScenario: "标的快速穿过目标区间或完全不动在远离 body 的位置，butterfly 失去 pin value。",
    whyClientDoes: ["用低 debit 表达一个明确下方目标价。", "希望 max loss 明确，不想买 outright put 的高 premium。", "将方向观点和 expiry pin/range 观点结合。"],
    clientType: "有明确 target price 和 expiry view 的 directional/range client。",
    suitability: "适合能接受盈利区间狭窄、execution legs 多、提前平仓可能受流动性影响的客户。",
    whenDealerSells: "客户买 Long Put Butterfly 时，dealer 卖出组合并承接 body strike 附近的局部 convexity 风险。",
    dealerExposure: "Dealer 管理三档 put strike 的 net Greeks，尤其是 body 附近的 Gamma flip 和 wing liquidity。",
    hedging: ["按 strike bucket 管理 Delta/Gamma，不只看 total delta。", "关注 body strike 附近 pin risk。", "报价时加入多腿 execution 和 wing liquidity 成本。"],
    profitSource: "多腿 bid-ask、body/wing 相对 skew、客户为低成本 target payoff 支付的结构化 spread。",
    commonMistakes: ["错误表达: Butterfly 便宜所以划算。正确说法是便宜来自盈利区间窄。", "错误表达: Long Put Butterfly 是纯看跌。正确说法是它更像 target/range payoff。", "错误表达: 到期才需要管理。正确说法是 body 附近 Gamma 和 liquidity 会提前影响退出。"],
    tradeoffAnswer: "它用有限 debit 买一个窄目标区间，牺牲了 outright put 的开放下行收益，换更低成本和更清晰的 max loss。",
    dealerAnswer: "Dealer 会看 body strike 的局部 Gamma 和两侧 wings 的流动性；如果 spot 靠近 body，hedging 和 unwind price 会更敏感。",
    mistakeAnswer: "最常见错误是把它讲成便宜 put。专业说法是 target-zone structure，不是全面 downside hedge。",
  }),

  "put-broken-wing": phase7aMemo({
    name: "Put Broken Wing",
    directional: "轻微看涨到区间偏强：希望 spot 不跌穿 short put body，同时利用不对称 lower wing。",
    volatility: "通常 short Vega：高 put skew 下卖出较贵的 downside optionality。",
    time: "+Theta：credit 或低 debit 结构依赖 short body 腿衰减。",
    convexity: "不对称 Gamma：一侧 wing 更宽，尾部风险比标准 butterfly 更偏。",
    makesMoneyFrom: "标的维持在 body 上方或靠近目标区间，short puts 衰减，broken wing 降低成本或形成 credit。",
    losesMoneyFrom: "标的跌向宽 wing 一侧，亏损集中在不对称尾部；流动性差会放大退出成本。",
    bestMarketCondition: "温和看涨或不大跌、put skew 偏高、希望用 broken wing 降低成本。",
    worstScenario: "gap down 穿过 lower wing，导致 max loss 快速接近且难以按理论价退出。",
    whyClientDoes: ["想做比普通 put butterfly 更低成本或 credit 的区间结构。", "愿意承担一侧更宽 wing 的尾部风险。", "利用 downside skew 设计 defined-risk income/range payoff。"],
    clientType: "熟悉 multi-leg execution 和 defined-risk tail trade-off 的高级客户。",
    suitability: "必须能解释 max loss 出现在宽 wing 一侧，并理解 assignment、liquidity 和 skew risk。",
    whenDealerSells: "客户做 Put Broken Wing 时，dealer 承接不对称 put wing 和 short body strike 风险。",
    dealerExposure: "Dealer 的 risk 不是对称 butterfly，而是偏向 downside wing 的 skew/Gamma/liquidity exposure。",
    hedging: ["拆分 body strike 与 broken wing 的 Delta/Gamma。", "对 gap-down scenario 做 stress。", "对 lower wing 的 liquidity 和 quote width 收费。"],
    profitSource: "Put skew、复杂结构 bid-ask、多腿 execution 和 wing asymmetry premium。",
    commonMistakes: ["错误表达: Broken Wing 是免费 butterfly。正确说法是用一侧更宽风险换低成本。", "错误表达: 有 long wing 就没有大风险。正确说法是 max loss 仍可能集中在宽 wing。", "错误表达: credit 就代表胜率高且安全。正确说法是 credit 是承担 tail shape 的补偿。"],
    tradeoffAnswer: "Put Broken Wing 用不对称 wing 换取低 debit 或 credit。核心不是免费收益，而是明确哪一侧尾部被放宽。",
    dealerAnswer: "Dealer 会重点看 downside wing 的 gap risk、put skew 和 unwind liquidity；这不是简单三腿/四腿 payoff 图能完全说明的风险。",
    mistakeAnswer: "最常见错误是说它是 free butterfly。专业表达应强调 wing asymmetry、max loss location、gap 和 assignment。",
  }),

  "inverse-call-broken-wing": phase7aMemo({
    name: "Inverse Call Broken Wing",
    directional: "偏空或反向区间观点：call wing 不对称，通常受 upside move 影响较大。",
    volatility: "取决于 net premium，但通常对 call skew 和 IV expansion 敏感。",
    time: "Theta profile 不稳定：spot 在 body 附近与远离 body 时符号可能不同。",
    convexity: "不对称 convexity：upside wing 的宽度决定尾部损益。",
    makesMoneyFrom: "标的不进入不利 upside 区间，或沿预期方向离开 short convexity 区域。",
    losesMoneyFrom: "标的快速向上穿过 call body/wing，结构进入最差区间。",
    bestMarketCondition: "有明确 resistance 或 upside move 受限观点，且 call wing 定价允许合理 risk/reward。",
    worstScenario: "强劲 short squeeze 或 event gap up，导致 upside wing 风险集中。",
    whyClientDoes: ["表达上方受限或反向区间观点。", "利用 call-side strikes 设计不对称 payoff。", "用 defined-risk multi-leg 代替 naked short call。"],
    clientType: "能理解 call-side tail 和多腿执行的高级 directional client。",
    suitability: "不适合只想简单收租的客户；必须能接受 payoff 非线性且不易直观解释。",
    whenDealerSells: "Dealer 承接 call-side broken wing 的 upside tail 和 body strike pin risk。",
    dealerExposure: "Upside rally 下 Delta/Gamma 变化会很快，call wing liquidity 和 borrow/corporate action 也会影响 hedge。",
    hedging: ["对 upside gap 做 stress。", "用 underlying 管理快速变化的 call Delta。", "监控 body strike 附近 pin 和 execution cost。"],
    profitSource: "Call wing skew、复杂结构报价 spread 和客户为不对称 payoff 支付的溢价。",
    commonMistakes: ["错误表达: inverse broken wing 和普通 broken wing 只是方向相反。正确说法是 tail location 和 Greeks path 也不同。", "错误表达: defined risk 所以不用管。正确说法是 mark-to-market 和 liquidity 仍可能很差。", "错误表达: call side 没有 assignment 重点。正确说法是 short ITM calls 仍有 dividend/assignment 关注点。"],
    tradeoffAnswer: "它用 call-side wing asymmetry 表达上方受限或反向区间观点。trade-off 是 payoff 难解释、tail location 必须讲清楚。",
    dealerAnswer: "Dealer 会把它拆成 call wing risk、body pin risk 和 upside gap scenario，而不是只看净 premium。",
    mistakeAnswer: "最常见错误是只说方向相反。专业表达必须讲清楚 inverse 结构的损益区间、tail side 和 hedge path。",
  }),

  "call-broken-wing": phase7aMemo({
    name: "Call Broken Wing",
    directional: "温和看涨或上方目标区间：收益通常集中在 call body 附近。",
    volatility: "通常 short Vega：希望目标区间附近 IV 不继续上升。",
    time: "+Theta near target：short body calls 的衰减有利。",
    convexity: "不对称 Gamma：上方 wing 宽度决定 capped/risk profile。",
    makesMoneyFrom: "标的上涨到目标区间但不过度突破，short call body 衰减并保留结构价值。",
    losesMoneyFrom: "标的远离目标区间，或强烈上涨进入 broken wing 的不利区域。",
    bestMarketCondition: "温和上涨到目标价、IV 不扩张、call skew 使结构成本合理。",
    worstScenario: "跳空上行越过上方 wing，或停在最不利 body/wing 区间。",
    whyClientDoes: ["用比 vertical 更精细的 payoff 表达目标价。", "希望降低 butterfly 成本。", "接受上方一侧 payoff 不对称。"],
    clientType: "有明确 upside target 的高级 options client。",
    suitability: "需要理解多腿执行、上方 capped payoff、max loss location 和 liquidity。",
    whenDealerSells: "Dealer 卖给客户 Call Broken Wing 后，重点管理 upside body 和 broken wing exposure。",
    dealerExposure: "Call-side Delta/Gamma、upside gap、short call assignment 和 wing liquidity 是主要风险。",
    hedging: ["spot 上涨接近 body 时提高 hedge 频率。", "检查 ex-dividend 前 short call assignment。", "按 call skew 调整 wing 报价。"],
    profitSource: "Call-side skew、multi-leg spread 和客户为 target-zone payoff 支付的结构化 edge。",
    commonMistakes: ["错误表达: Call Broken Wing 是便宜 bull call spread。正确说法是它有更复杂的 body/wing 风险。", "错误表达: 只要看涨就适合。正确说法是需要目标价和路径判断。", "错误表达: 上方风险完全封顶所以不用管。正确说法是最差区间和 liquidity 仍要 stress。"],
    tradeoffAnswer: "它用更复杂的 payoff 换更低成本或更好的 target-zone economics，但牺牲简单 vertical 的可解释性。",
    dealerAnswer: "Dealer 关注 upside gap、short call assignment 和 body strike 附近的 Gamma concentration。",
    mistakeAnswer: "最常见错误是把它说成便宜 vertical。专业表达应强调 target zone、broken wing 和 max loss location。",
  }),

  "inverse-put-broken-wing": phase7aMemo({
    name: "Inverse Put Broken Wing",
    directional: "偏多或反向 downside 区间观点：put wing 不对称，风险集中在下方路径。",
    volatility: "对 put skew 和 downside IV expansion 敏感。",
    time: "Theta profile 取决于 spot 与 body 的距离，不能简单说收租。",
    convexity: "不对称 downside convexity：下跌接近 body/wing 时 Greeks 快速变化。",
    makesMoneyFrom: "标的不进入不利 downside 区间，或按预期离开 short convexity 区域。",
    losesMoneyFrom: "快速下跌进入 put broken wing 的最差区间，尤其是 gap-down。",
    bestMarketCondition: "有明确 support 或 downside 受限观点，且 put skew 让结构有合理补偿。",
    worstScenario: "跳空下跌、put IV 飙升、bid-ask 变宽，导致理论 defined risk 也难退出。",
    whyClientDoes: ["表达下方支撑或反向区间观点。", "利用 put strikes 构建不对称 defined-risk payoff。", "避免 naked short put，但保留更复杂的收益区间。"],
    clientType: "高级 risk-budgeted client。",
    suitability: "需要能解释 downside tail、assignment、skew 和 multi-leg liquidity。",
    whenDealerSells: "Dealer 承接 put-side inverse broken wing 的 downside convexity 和 skew risk。",
    dealerExposure: "Downside move 下 put Delta/Gamma 与 IV 同时恶化，hedge 和 unwind 成本可能同步上升。",
    hedging: ["对 gap-down + IV up 做联合 stress。", "用 underlying 动态调整 net Delta。", "管理 short put assignment 和 lower wing liquidity。"],
    profitSource: "Put skew、复杂结构报价和客户为非对称 downside payoff 支付的 edge。",
    commonMistakes: ["错误表达: inverse put broken wing 是普通 put broken wing 的镜像。正确说法是 path、tail 和 Greeks 都要重新拆。", "错误表达: 有 wing 所以不会出大事。正确说法是 gap/liquidity 会让退出远离理论价。", "错误表达: credit 结构就是收入策略。正确说法是 credit 是承担 downside shape 的补偿。"],
    tradeoffAnswer: "它通过 put-side asymmetry 表达下方支撑/反向区间观点，trade-off 是 downside path risk 和解释复杂度。",
    dealerAnswer: "Dealer 会把它当成 downside skew + Gamma + assignment 的组合风险，尤其看 gap-down scenario。",
    mistakeAnswer: "最常见错误是简单叫镜像结构。专业回答应重新定位 tail、max loss、skew 和 hedge path。",
  }),

  "covered-short-straddle": phase7aMemo({
    name: "Covered Short Straddle",
    directional: "持股带来 +Delta，short call/short put 叠加 short volatility 和额外 downside obligation。",
    volatility: "-Vega：IV 回落有利，IV 上升不利。",
    time: "+Theta：收取 straddle premium，但用 short Gamma 换来。",
    convexity: "-Gamma：spot 大幅波动会让组合快速恶化。",
    makesMoneyFrom: "标的围绕 strike 横盘，short call 和 short put 同时衰减，同时保留部分持股收益。",
    losesMoneyFrom: "大跌时持股亏损加 short put 接货风险；大涨时 short call 封顶并可能 assignment。",
    bestMarketCondition: "高 IV 后进入低波动横盘，且客户愿意在 strike 附近买/卖股票。",
    worstScenario: "大幅下跌，持股亏损和 short put obligation 同时出现。",
    whyClientDoes: ["希望在持股基础上提高 premium income。", "认为标的短期会横盘。", "愿意在下跌时加仓、上涨时被 call away。"],
    clientType: "有持股基础且非常理解 assignment 的 income client。",
    suitability: "只适合能承担股票大幅下跌、追加接货和 upside cap 的客户。",
    whenDealerSells: "客户建立 Covered Short Straddle 时，dealer 站在 long straddle 风险的另一侧或提供执行。",
    dealerExposure: "Dealer 会关注客户 short vol 的 suitability，同时自身 book 管理 long/short gamma offset。",
    hedging: ["分别评估 stock、short call、short put 的 Delta。", "对 downside stock + put assignment 做 stress。", "监控 ex-dividend call assignment 和 margin。"],
    profitSource: "Execution spread、vol risk premium 和客户 short-vol flow 的 bid-ask。",
    commonMistakes: ["错误表达: covered 表示完全对冲。正确说法是 stock 只覆盖 short call，不覆盖 short put downside。", "错误表达: straddle premium 很厚所以安全。正确说法是厚 premium 补偿大波动风险。", "错误表达: 最坏只是被卖出股票。正确说法是大跌时可能持股亏损并被 put assignment。"],
    tradeoffAnswer: "它用很高的 short-vol premium 换取双向 obligation：上涨被封顶，下跌可能加仓并承受持股亏损。",
    dealerAnswer: "Dealer 和 risk manager 会先问 suitability、margin、assignment 和 downside stress，而不是只看 premium income。",
    mistakeAnswer: "最常见错误是把 covered 理解成 hedged。专业表达必须说清 stock 只覆盖 call side，put side 仍是额外 downside。",
  }),

  "covered-short-strangle": phase7aMemo({
    name: "Covered Short Strangle",
    directional: "+Delta from stock，叠加 OTM short call 和 OTM short put 的 range income。",
    volatility: "-Vega：短波动，IV 上升或尾部需求上升不利。",
    time: "+Theta：依赖两个 OTM options 衰减。",
    convexity: "-Gamma：大涨或大跌都不利，尤其下跌端有持股和 short put 双重风险。",
    makesMoneyFrom: "标的留在 short put 与 short call 之间，两个 OTM options 衰减。",
    losesMoneyFrom: "跌破 short put 时加仓/亏损，涨破 short call 时持股被 call away 且 upside 被封顶。",
    bestMarketCondition: "温和区间、高 IV、客户愿意按上下 strike 执行股票交易。",
    worstScenario: "快速下跌穿过 put strike，持股亏损和 short put assignment 同时出现。",
    whyClientDoes: ["想比 covered call 收更多 premium。", "愿意在更低价加仓，并在更高价卖出股票。", "认为短期会维持宽区间。"],
    clientType: "经验丰富的 stock income client。",
    suitability: "必须有股票风险承受能力、现金/保证金能力和 assignment 处理计划。",
    whenDealerSells: "Dealer 执行 Covered Short Strangle flow 时，会重点提示 short put 并非 covered by stock。",
    dealerExposure: "Short wings 的 Gamma/Vega、stock Delta、assignment 和 liquidity 共同决定风险。",
    hedging: ["分开 stress upside call-away 和 downside put assignment。", "监控 portfolio concentration。", "用 Delta 和 vega bucket 看整体 short-vol exposure。"],
    profitSource: "Bid-ask、short-vol risk premium 和多腿执行 spread。",
    commonMistakes: ["错误表达: 比 covered call 只是多收一点租。正确说法是多卖一个 put，新增 downside obligation。", "错误表达: 两边 OTM 所以风险低。正确说法是 gap move 会让 OTM 迅速变 ITM。", "错误表达: 持股覆盖整个结构。正确说法是持股只覆盖 short call。"],
    tradeoffAnswer: "它比 covered call 收更多 premium，但新增 short put downside obligation；这是 income vs tail risk 的交换。",
    dealerAnswer: "Dealer 会看客户是否真愿意在 put strike 买更多股票，并检查 margin、concentration 和 assignment。",
    mistakeAnswer: "最常见错误是说持股覆盖了 strangle。专业表达要拆开 call side 和 put side 的不同 obligation。",
  }),

  "short-call-condor": phase7aMemo({
    name: "Short Call Condor",
    directional: "近似中性 range credit：希望标的不要进入 condor 的不利区间。",
    volatility: "-Vega：IV 回落有利，IV 上升会推高回补成本。",
    time: "+Theta：short middle call spread 的衰减是主要收益来源。",
    convexity: "-Gamma around body：接近中间 strikes 时风险最敏感。",
    makesMoneyFrom: "标的留在预期区间外/有利区间，call condor credit 衰减。",
    losesMoneyFrom: "标的停在 short call body 附近，结构接近 max loss。",
    bestMarketCondition: "高 IV、预期不会 pin 在中间 short strikes、call-side liquidity 好。",
    worstScenario: "expiry 附近 pin 在 short strikes，assignment 和 execution cost 同时上升。",
    whyClientDoes: ["用 calls 表达 range/short-vol view。", "希望 max loss 有定义。", "利用 call-side strikes 和 margin efficiency。"],
    clientType: "熟悉 defined-risk short-vol 的 advanced client。",
    suitability: "需要理解多腿 assignment、pin risk、call liquidity 和 max loss。",
    whenDealerSells: "Dealer 对客户 Short Call Condor flow 报价时，会按四个 call strikes 管理 risk。",
    dealerExposure: "Short middle strikes 的 Gamma、long wings 的 liquidity 和 early assignment 是核心。",
    hedging: ["按 strike bucket 管理 call Delta。", "expiry 周监控 pin/assignment。", "对宽 bid-ask 的 wings 加执行成本。"],
    profitSource: "四腿执行 spread、call skew 和客户为 defined-risk short-vol 支付的 liquidity premium。",
    commonMistakes: ["错误表达: Condor defined risk 所以不用管理。正确说法是 short strikes 附近 pin risk 很重要。", "错误表达: 用 calls 或 puts 都一样。正确说法是 assignment、skew 和 liquidity 会不同。", "错误表达: max loss 小就可以放大仓位。正确说法是 position sizing 应按 stress loss 和 exit cost。"],
    tradeoffAnswer: "Short Call Condor 用 capped max loss 换 capped premium，核心 trade-off 是收 theta 但承担 body strikes 附近的 short Gamma。",
    dealerAnswer: "Dealer 会按四个 call strikes 看 net Greeks，尤其 expiry 前 short strikes 的 pin 和 assignment。",
    mistakeAnswer: "最常见错误是忽略 pin risk。专业表达要说明 max loss、body strikes、assignment 和 liquidity。",
  }),

  "short-put-condor": phase7aMemo({
    name: "Short Put Condor",
    directional: "近似中性 range credit，但受 put skew 和 downside gap 影响更明显。",
    volatility: "-Vega：卖出中间 put premium，IV 回落有利。",
    time: "+Theta：希望 short put body 腿衰减。",
    convexity: "-Gamma around body：spot 接近 short put strikes 时风险集中。",
    makesMoneyFrom: "标的不落在不利 put body 区间，premium 随时间衰减。",
    losesMoneyFrom: "标的下跌或 pin 在 short put strikes 附近，结构走向 max loss。",
    bestMarketCondition: "高 put IV、下方支撑明确、四腿流动性可接受。",
    worstScenario: "gap down + IV spike + put bid-ask widening，导致理论风险和执行风险同时恶化。",
    whyClientDoes: ["用 puts 表达 defined-risk range income。", "利用 put skew 收取更高 premium。", "希望避免 naked short put 的开放尾部。"],
    clientType: "理解 put skew 和 assignment 的 defined-risk income client。",
    suitability: "适合能解释 downside scenario、early assignment 和 multi-leg execution 的客户。",
    whenDealerSells: "Dealer 对 Short Put Condor flow 需要管理四个 put strikes 的 skew 和 assignment。",
    dealerExposure: "Put skew、downside Gamma、short put assignment 和 long wing liquidity 是主要风险。",
    hedging: ["downside move 时动态调整 Delta。", "监控 short puts 的 assignment 风险。", "用 stress 检查 gap + IV up。"],
    profitSource: "Put skew、四腿 bid-ask 和 defined-risk income demand 的 liquidity premium。",
    commonMistakes: ["错误表达: Short Put Condor 和 Short Call Condor 完全等价。正确说法是 put skew 和 assignment 改变风险。", "错误表达: long wings 消除 crash worry。正确说法是 crash 时 liquidity 和 IV spike 仍会伤害退出。", "错误表达: 收 credit 就是稳定收益。正确说法是 credit 是 short Gamma/short Vega 的补偿。"],
    tradeoffAnswer: "它用 defined-risk condor 收 theta 和 skew premium，但主要风险集中在 downside body strikes。",
    dealerAnswer: "Dealer 会特别看 put skew、gap-down stress 和 short put assignment，而不是只看 payoff 图。",
    mistakeAnswer: "最常见错误是把 call/put condor 当完全一样。专业表达要加入 skew、assignment 和 liquidity。",
  }),

  "reverse-jade-lizard": phase7aMemo({
    name: "Reverse Jade Lizard",
    directional: "通常偏空或反向 skew structure：组合方向取决于 call/put spread placement。",
    volatility: "对 skew 敏感：不是简单 short vol，取决于哪一侧卖得更贵。",
    time: "Theta profile 取决于 net credit/debit 和 spot 区间。",
    convexity: "一侧 defined spread，另一侧 option leg 造成不对称 Gamma。",
    makesMoneyFrom: "标的按预期方向移动或留在有利区间，同时卖出的 premium 衰减。",
    losesMoneyFrom: "标的向不利方向突破，尤其是 naked/less-protected side 扩大亏损。",
    bestMarketCondition: "有明确 directional + skew view，并能解释哪一侧风险被保留。",
    worstScenario: "市场向 uncovered 或 less-protected side gap，导致亏损超出简单 credit spread 直觉。",
    whyClientDoes: ["表达比普通 Jade Lizard 更反向的 skew/directional view。", "用 premium 组合降低 upfront cost。", "接受一侧更复杂的 payoff。"],
    clientType: "高级 options client，不适合初学收租账户。",
    suitability: "必须能清楚展示 payoff、max loss/开放风险、margin 和 assignment。",
    whenDealerSells: "Dealer 需要把 reverse jade lizard 拆成 vertical spread 加 short/long option leg。",
    dealerExposure: "方向、skew、Gamma 和 margin impact 不能用单一 label 概括。",
    hedging: ["画出 payoff 并定位 worst zone。", "分别 hedge spread 和单腿 option。", "监控不利方向 gap 与 assignment。"],
    profitSource: "复杂结构报价、skew differential 和客户为定制 payoff 支付的 spread。",
    commonMistakes: ["错误表达: Reverse Jade Lizard 也是 free lunch。正确说法是 premium 来自保留某一侧风险。", "错误表达: 名字说明风险。正确说法是必须拆 legs 和 payoff。", "错误表达: 只看 credit 大小。正确说法是先看 worst-case 和 margin。"],
    tradeoffAnswer: "它用结构复杂度和一侧风险换 premium/成本优势。核心是先解释 legs，再解释哪一侧风险没有被完全保护。",
    dealerAnswer: "Dealer 会拆成 spread + option leg 管理，不会因为名字是 lizard 就按固定模板报价。",
    mistakeAnswer: "最常见错误是把它当 Jade Lizard 的小变体。专业表达要重建 payoff、risk side 和 margin。",
  }),

  "call-ratio-spread": phase7aMemo({
    name: "Call Ratio Spread",
    directional: "温和看涨到目标价；超过 short calls 后可能变成上方 short tail。",
    volatility: "通常 short Vega：卖出更多 OTM calls，IV 上升不利。",
    time: "+Theta near short calls，但 spot 快速上涨时 short Gamma 主导。",
    convexity: "先 long Gamma 后 short Gamma：超过 short strikes 后 upside convexity 变差。",
    makesMoneyFrom: "标的上涨到 short call 区域但不过度突破，同时卖出 calls 补贴 long call 成本。",
    losesMoneyFrom: "标的大幅上涨，额外 short calls 造成开放或很大的 upside loss。",
    bestMarketCondition: "温和上涨、有上方目标、call skew/IV 使 short calls 有吸引力。",
    worstScenario: "short squeeze 或 takeover gap up，short call ratio tail 快速扩大。",
    whyClientDoes: ["想低成本表达有限上涨。", "认为标的会涨到目标价但不会暴涨。", "愿意用上方 tail risk 换取更低 premium。"],
    clientType: "高级 directional client，不能只按普通 debit spread 理解。",
    suitability: "必须能承担或定义上方 short call tail；不适合无法接受 margin 扩张的客户。",
    whenDealerSells: "Dealer 卖给客户 ratio spread 时，重点看客户是否理解额外 short calls。",
    dealerExposure: "Dealer 的 hedge 会随 spot 上涨快速变化，short call tail、borrow/dividend 和 liquidity 重要。",
    hedging: ["对 upside gap 做 stress。", "接近 short calls 时提高 Delta/Gamma 管理频率。", "检查是否需要用 extra wing 转成 defined-risk ratio。"],
    profitSource: "Call skew、ratio execution spread 和客户为低 upfront upside structure 支付的 edge。",
    commonMistakes: ["错误表达: Call Ratio Spread 是便宜 bull call spread。正确说法是便宜来自额外 short call tail。", "错误表达: 上涨一定赚钱。正确说法是过度上涨会反而亏损。", "错误表达: 只要有 long call 就保护。正确说法是 long call 不能覆盖全部 short call 数量。"],
    tradeoffAnswer: "它用额外 short calls 补贴 long call，因此在目标区间很好，但暴涨会暴露上方 ratio tail。",
    dealerAnswer: "Dealer 会看 upside gap、short call assignment、borrow/dividend 和是否需要建议客户加 wing 限定风险。",
    mistakeAnswer: "最常见错误是叫它 cheap call spread。专业表达必须指出 ratio tail 和超过 short strikes 后的风险反转。",
  }),

  "put-ratio-spread": phase7aMemo({
    name: "Put Ratio Spread",
    directional: "温和看跌到目标价；跌破 short puts 后变成 downside short tail。",
    volatility: "通常 short Vega，且对 put skew/IV spike 特别敏感。",
    time: "+Theta near short puts，但快速下跌时 short Gamma 和 Vega 会主导亏损。",
    convexity: "目标区间前有 convexity，跌破 short puts 后 downside convexity 变差。",
    makesMoneyFrom: "标的下跌到 short put 区域但不崩盘，卖出 puts 补贴 long put 成本。",
    losesMoneyFrom: "标的大幅下跌，额外 short puts 带来接货/尾部亏损。",
    bestMarketCondition: "温和下跌、有下方目标、put skew 让 short puts 能有效补贴 long put。",
    worstScenario: "crash gap down，put IV 飙升，short put ratio tail 和 liquidity 同时恶化。",
    whyClientDoes: ["想低成本表达有限下跌。", "认为会跌到目标价但不会崩盘。", "愿意用更低 strike 的 short put obligation 换成本优势。"],
    clientType: "高级 bearish/hedging client。",
    suitability: "必须能承担 short put assignment、margin 和 crash scenario；不适合简单保护需求。",
    whenDealerSells: "Dealer 会把 Put Ratio Spread 当成 long put plus extra short puts，而不是 cheap hedge。",
    dealerExposure: "Downside gap、put skew、short put assignment 和 crash liquidity 是主要风险。",
    hedging: ["对 crash + IV up 做联合 stress。", "监控 short puts 的 Delta/Gamma 加速。", "必要时建议加 lower wing 变成 defined-risk put spread。"],
    profitSource: "Put skew、ratio execution spread 和客户为低成本 downside target 支付的 edge。",
    commonMistakes: ["错误表达: Put Ratio Spread 是便宜 protective put。正确说法是它卖出额外 downside tail。", "错误表达: 下跌越多越好。正确说法是跌过 short puts 后风险会反转。", "错误表达: short puts 很远所以不用管。正确说法是 crash 时远 OTM 会迅速变成核心风险。"],
    tradeoffAnswer: "它用额外 short puts 降低 long put 成本，因此适合目标式下跌观点，不适合灾难保护。",
    dealerAnswer: "Dealer 会重点看 crash gap、put skew 和 assignment；这类结构不能卖给只想买保险的客户而不解释 tail。",
    mistakeAnswer: "最常见错误是说它是 cheap hedge。专业表达要明确它牺牲了极端下跌保护。",
  }),

  "bull-call-ladder": phase7aMemo({
    name: "Bull Call Ladder",
    directional: "+Delta 到目标区间，超过额外 short call 后 upside risk 会反转。",
    volatility: "通常 -Vega：卖出两档上方 calls 补贴 long call。",
    time: "+Theta near short calls，但暴涨时 short Gamma 主导。",
    convexity: "先 long Gamma，超过 ladder 上方后变成 short upside convexity。",
    makesMoneyFrom: "标的温和上涨到目标区间，long call 变 ITM，而 short calls 衰减或限制成本。",
    losesMoneyFrom: "标的暴涨超过上方 short calls，额外 short call exposure 造成上方 tail loss。",
    bestMarketCondition: "温和上涨、有明确 upside target、预计不会出现 squeeze 或 takeover gap。",
    worstScenario: "跳空大涨穿过所有 call strikes，short call ladder tail 迅速扩大。",
    whyClientDoes: ["用低成本表达有限上涨。", "认为上方目标明确，暴涨概率低。", "愿意用 capped/反转 upside 换低 premium。"],
    clientType: "高级 directional client，不能只看 bull 字样。",
    suitability: "必须理解上方可能出现开放或很大的 short call risk，以及 margin 和 assignment。",
    whenDealerSells: "Dealer 报 Bull Call Ladder 时，会先拆成 long call spread 加额外 short call。",
    dealerExposure: "Upside gap、short call assignment、call skew 和 borrow/dividend 是主要风险。",
    hedging: ["spot 接近 short calls 时提高 Delta/Gamma 管理。", "对 takeover-style gap up 做 stress。", "必要时建议加 long call wing 定义上方风险。"],
    profitSource: "Call-side skew、ratio/ladder execution spread 和客户为低成本 upside view 支付的 edge。",
    commonMistakes: ["错误表达: Bull Call Ladder 是更便宜的 bull call spread。正确说法是便宜来自额外 short call tail。", "错误表达: 看涨策略上涨越多越好。正确说法是超过上方 short calls 后风险反转。", "错误表达: long call 覆盖全部风险。正确说法是 long call 数量不足以覆盖额外 short calls。"],
    tradeoffAnswer: "Bull Call Ladder 用额外 short call 降低成本，适合温和上涨目标，但不适合无上限看涨。",
    dealerAnswer: "Dealer 会看上方 gap、short call assignment 和是否需要加 wing 限制 tail。",
    mistakeAnswer: "最常见错误是把 ladder 当普通 vertical。专业表达必须指出额外 short call 和 upside tail。",
  }),

  "bear-call-ladder": phase7aMemo({
    name: "Bear Call Ladder",
    directional: "初始偏空或收 credit，但上方大涨时可能转为复杂 upside exposure。",
    volatility: "常见为 short-to-mixed Vega，取决于 ladder strikes 和 net premium。",
    time: "+Theta near sold call area，但路径偏离后可能不稳定。",
    convexity: "Call ladder 让 convexity 分段变化，不能用单一 delta 描述。",
    makesMoneyFrom: "标的不涨破 short call 区域，或按预期在较低区间停留。",
    losesMoneyFrom: "标的强势上涨进入 ladder 不利区间，short call exposure 扩大。",
    bestMarketCondition: "上方阻力明确、IV 较高、预期上涨受限。",
    worstScenario: "强势 squeeze 上行，short calls 变成主要风险。",
    whyClientDoes: ["想用 call-side premium 表达上方受限。", "愿意用复杂 payoff 换更低成本或 credit。", "认为上方 tail 可控。"],
    clientType: "高级 short-call-side structure client。",
    suitability: "必须能展示 payoff 图、max loss/开放风险、assignment 和 liquidity。",
    whenDealerSells: "Dealer 会拆成 call spread 与额外 call leg，并检查上方 tail。",
    dealerExposure: "Upside Delta/Gamma、short call assignment 和 call liquidity 是关键。",
    hedging: ["对 upside gap 做 scenario。", "关注 ex-dividend 前 short calls。", "用 strike-level Greeks 管理，不只看 net Greeks。"],
    profitSource: "Call skew、执行 spread 和客户定制上方 payoff 的结构化费用。",
    commonMistakes: ["错误表达: Bear Call Ladder 是普通 bear call spread。正确说法是 ladder 多出的腿改变上方 tail。", "错误表达: 收 credit 就一定偏安全。正确说法是 credit 补偿 short call-side risk。", "错误表达: 只要观点偏空就适合。正确说法是也要判断上方 gap risk。"],
    tradeoffAnswer: "它用 call ladder 的复杂结构表达上方受限观点，但 trade-off 是 payoff 不如 vertical 直观。",
    dealerAnswer: "Dealer 会重点管理上方 call tail 和 assignment，而不是只看初始 credit。",
    mistakeAnswer: "最常见错误是把它当 bear call spread。专业表达必须解释 ladder legs 和上方不利区间。",
  }),

  "bull-put-ladder": phase7aMemo({
    name: "Bull Put Ladder",
    directional: "偏多到区间观点，但下方额外 short put 会形成 downside obligation。",
    volatility: "通常 -Vega，且对 put skew 和 IV spike 敏感。",
    time: "+Theta near sold puts，但下跌时 short Gamma 主导。",
    convexity: "下方路径变成 short downside convexity。",
    makesMoneyFrom: "标的不跌破 put ladder 的主要 short strike，premium 衰减。",
    losesMoneyFrom: "标的大跌穿过额外 short puts，downside tail 扩大。",
    bestMarketCondition: "温和看涨或下方支撑明确、put skew 高、愿意承担接货风险。",
    worstScenario: "crash gap down，short put ladder tail 和 margin 同时恶化。",
    whyClientDoes: ["用 put premium 表达下方支撑。", "降低成本或收 credit。", "愿意在较低价承担额外买入义务。"],
    clientType: "高级 income/directional client。",
    suitability: "必须能承担 downside assignment、margin expansion 和 crash scenario。",
    whenDealerSells: "Dealer 会把 Bull Put Ladder 拆成 put spread 加额外 short put。",
    dealerExposure: "Downside gap、put skew、assignment 和 liquidity 是主要风险。",
    hedging: ["对 gap-down + IV up 做 stress。", "监控 short puts 的 Delta/Gamma 加速。", "必要时用 lower put wing 定义风险。"],
    profitSource: "Put skew、ladder execution spread 和客户为低成本 bullish structure 支付的 edge。",
    commonMistakes: ["错误表达: Bull Put Ladder 是更高收益 bull put spread。正确说法是收益来自新增 downside obligation。", "错误表达: 下方 strike 很远所以不用管。正确说法是 crash 会让远 OTM put 快速变核心风险。", "错误表达: 可以按 credit 大小 sizing。正确说法是按 worst-case 和 margin sizing。"],
    tradeoffAnswer: "它用额外 short put 换取更低成本/更高 credit，适合下方支撑观点，不适合不愿接货的客户。",
    dealerAnswer: "Dealer 会强调 downside stress、assignment 和 margin，而不是只看 premium。",
    mistakeAnswer: "最常见错误是把它当增强版 bull put spread。专业表达要讲额外 short put tail。",
  }),

  "bear-put-ladder": phase7aMemo({
    name: "Bear Put Ladder",
    directional: "看跌到目标区间，但过度下跌后 extra short put 会让收益反转。",
    volatility: "通常 -Vega after financing：卖出更多下方 puts 补贴 long put。",
    time: "+Theta near short puts，但 crash 时 Vega/Gamma 不利。",
    convexity: "有限下跌受益，极端下跌转为 short downside convexity。",
    makesMoneyFrom: "标的下跌到目标区间但不崩盘，long put 获利且 short puts 未造成过大损失。",
    losesMoneyFrom: "标的跌破额外 short puts，tail loss 或 assignment 变成主要风险。",
    bestMarketCondition: "温和到中等下跌、有明确下方目标、put skew 让融资有效。",
    worstScenario: "市场崩盘，short lower puts 暴露 crash tail。",
    whyClientDoes: ["想低成本表达下跌到目标价。", "认为会跌但不会 crash。", "愿意卖出更低 strike puts 来融资。"],
    clientType: "高级 bearish client，不适合纯保护需求。",
    suitability: "不适合只想买保险的客户；必须理解极端下跌保护被牺牲。",
    whenDealerSells: "Dealer 会把 Bear Put Ladder 解释为 put spread 加额外 short put。",
    dealerExposure: "Crash gap、put skew、short put assignment 和 liquidity 是核心。",
    hedging: ["对 crash + IV spike 做联合 stress。", "监控 lower short puts。", "必要时建议加 far downside wing。"],
    profitSource: "Put skew、ratio/ladder execution spread 和定制 downside target 的报价 edge。",
    commonMistakes: ["错误表达: Bear Put Ladder 是便宜 protective put。正确说法是它卖出了 extreme downside。", "错误表达: 下跌越多越好。正确说法是跌过 lower shorts 后风险反转。", "错误表达: skew 高就是机会。正确说法是 skew 高也说明 crash protection 贵。"],
    tradeoffAnswer: "它降低 bearish trade 成本，但牺牲灾难保护，适合目标式下跌，不适合 hedge tail。",
    dealerAnswer: "Dealer 会重点看 lower short puts 的 crash exposure 和 assignment。",
    mistakeAnswer: "最常见错误是把它作为 cheap insurance。专业表达要说清 extreme downside 被卖掉。",
  }),

  "short-synthetic-future": phase7aMemo({
    name: "Short Synthetic Future",
    directional: "synthetic short：short call + long put 近似复制 short forward。",
    volatility: "Vega 近似抵消，但 skew、strike 和 expiry mismatch 会留下 residual vega。",
    time: "Theta 近似抵消，但 carry、rates、dividends 和 borrow 会影响 forward value。",
    convexity: "Gamma 近似抵消，主要像线性 short delta。",
    makesMoneyFrom: "标的下跌带来近似线性收益，也可能来自 synthetic 与 forward/cash 的 mispricing。",
    losesMoneyFrom: "标的上涨造成线性亏损；borrow、dividend、assignment 或 execution cost 会破坏理论 parity。",
    bestMarketCondition: "需要用 options 表达 short exposure，或做 conversion/reversal/parity 相关分析。",
    worstScenario: "标的大涨且 short call assignment 或 borrow/corporate action 让 hedge 复杂化。",
    whyClientDoes: ["用 options 构建 short delta。", "避免或替代直接 short stock 的 borrow/friction。", "表达 parity 或 financing view。"],
    clientType: "专业客户、对冲账户或结构化交易账户。",
    suitability: "必须理解它近似 short stock，亏损可随标的上涨线性扩大。",
    whenDealerSells: "Dealer 看到 synthetic short 会按 forward exposure 管理，而不是按两个独立 options 管理。",
    dealerExposure: "主要是 short delta/forward risk，叠加 rates、dividends、borrow、assignment 和 parity basis。",
    hedging: ["用 stock/forward 对冲净 delta。", "检查 put-call parity、dividend 和 borrow。", "监控 short call assignment。"],
    profitSource: "Execution spread、financing basis、borrow/dividend assumptions 和 parity mispricing。",
    commonMistakes: ["错误表达: Synthetic short 比 short stock 风险小。正确说法是经济暴露近似 short stock。", "错误表达: Vega/Gamma 完全为零。正确说法是实际 strikes、skew 和 execution 会留下 residual risk。", "错误表达: parity 一定可套利。正确说法是要扣除 bid-ask、borrow、dividend、margin 和 assignment。"],
    tradeoffAnswer: "Short Synthetic Future 用 options 复制 short forward，trade-off 是获得结构灵活性但引入 parity inputs 和 assignment 细节。",
    dealerAnswer: "Dealer 会按 synthetic short 的 forward delta 管理，同时检查 carry、dividend、borrow 和 parity basis。",
    mistakeAnswer: "最常见错误是说 synthetic 比现货更安全。专业表达应说它复制线性 short exposure，只是实现路径不同。",
  }),

  "synthetic-put": phase7aMemo({
    name: "Synthetic Put",
    directional: "Long call + short stock 复制 long put 的下行 convexity。",
    volatility: "+Vega：经济上接近 long put。",
    time: "-Theta：为 downside convexity 支付 time value/carry。",
    convexity: "+Gamma：标的下跌时保护价值上升。",
    makesMoneyFrom: "标的下跌、IV 上升，或 synthetic put 相对 listed put 更有效率。",
    losesMoneyFrom: "标的上涨、IV 回落、borrow/carry 和 option theta 消耗。",
    bestMarketCondition: "需要 put-like payoff，但直接 put 流动性/报价/borrow 组合更适合拆分执行。",
    worstScenario: "标的上涨且 IV 回落，同时 short stock borrow 成本不利。",
    whyClientDoes: ["用 call + short stock 合成 put-like protection。", "在 put 流动性差时拆分执行。", "利用 parity 或 borrow/carry view。"],
    clientType: "专业 hedger、relative-value client。",
    suitability: "需要能 short stock 或获得 borrow，并理解合成结构不是免费替代。",
    whenDealerSells: "Dealer 会把 synthetic put 还原成 put exposure，并管理 borrow/carry。",
    dealerExposure: "Put-like long vega/gamma，叠加 stock borrow、dividend 和 execution basis。",
    hedging: ["确认 stock borrow 和 dividend assumptions。", "用 parity 检查 synthetic 与 listed put 的相对价值。", "管理 call leg 的 vega/gamma。"],
    profitSource: "Parity basis、execution spread 和 borrow/carry assumptions。",
    commonMistakes: ["错误表达: Synthetic put 和 listed put 永远一样。正确说法是现实中 bid-ask、borrow 和 dividends 会造成差异。", "错误表达: short stock leg 只是技术细节。正确说法是 borrow/corporate action 可能主导结果。", "错误表达: 便宜就一定套利。正确说法是要做 full carry check。"],
    tradeoffAnswer: "Synthetic Put 提供 put-like payoff，但 trade-off 是引入 short stock borrow、dividend 和 execution complexity。",
    dealerAnswer: "Dealer 会用 parity 还原风险，并检查 borrow/carry，而不是只看 option leg。",
    mistakeAnswer: "最常见错误是忽略 short stock leg。专业表达要把 borrow 和 carry 放进解释。",
  }),

  "long-combo": phase7aMemo({
    name: "Long Combo",
    directional: "+Delta：通常 long call + short put，近似 synthetic long forward。",
    volatility: "Vega 近似抵消，但 skew 和 moneyness 会留下 residual vega。",
    time: "Theta/carry 取决于 strikes、rates、dividends 和 net premium。",
    convexity: "Gamma 近似抵消，主要是线性 long exposure。",
    makesMoneyFrom: "标的上涨，或 synthetic forward 相对 cash/forward 定价更有利。",
    losesMoneyFrom: "标的下跌，short put 可能带来 assignment 和 margin pressure。",
    bestMarketCondition: "想用 options 建立低 upfront long delta，并能承担 downside obligation。",
    worstScenario: "标的大跌，short put 变成核心风险，保证金和接货压力上升。",
    whyClientDoes: ["用 options 合成 long exposure。", "降低 upfront cash outlay。", "表达 risk reversal/forward view。"],
    clientType: "能承担 short put downside 的 directional client。",
    suitability: "不适合只想有限亏损看涨的客户；short put obligation 必须披露。",
    whenDealerSells: "Dealer 会把 long combo 视为 synthetic long forward/risk reversal exposure。",
    dealerExposure: "Forward delta、put skew、assignment 和 margin 是主要风险。",
    hedging: ["按 synthetic forward delta hedge。", "监控 short put assignment。", "检查 skew 造成的 entry economics。"],
    profitSource: "Skew、financing basis 和 multi-leg execution spread。",
    commonMistakes: ["错误表达: Long Combo 是便宜 long call。正确说法是 short put 融资引入 downside obligation。", "错误表达: 最大亏损是 premium。正确说法是亏损接近 long stock downside。", "错误表达: risk reversal 永远适合看涨。正确说法是要看客户是否愿意接货。"],
    tradeoffAnswer: "Long Combo 用 short put 融资 long call，换来低 upfront long delta，但 downside 不再有限。",
    dealerAnswer: "Dealer 会按 synthetic forward 管理 delta，并重点看 short put skew 和 assignment。",
    mistakeAnswer: "最常见错误是把它讲成便宜 call。专业表达要说清 short put financing 和 downside obligation。",
  }),

  "short-combo": phase7aMemo({
    name: "Short Combo",
    directional: "-Delta：通常 short call + long put，近似 synthetic short forward。",
    volatility: "Vega 近似抵消，但 skew、strikes 和 execution 留下 residual risk。",
    time: "Carry 取决于 rates、dividends、borrow 和 net premium。",
    convexity: "Gamma 近似抵消，主要像线性 short exposure。",
    makesMoneyFrom: "标的下跌，或 synthetic short forward 定价优于直接 short stock。",
    losesMoneyFrom: "标的上涨，short call 造成上行亏损和 assignment 风险。",
    bestMarketCondition: "需要建立 short delta，又想通过 options 管理 borrow/carry 或结构限制。",
    worstScenario: "标的大涨、short call assignment、borrow/corporate action 造成 hedge 复杂化。",
    whyClientDoes: ["用 options 表达 short view。", "管理 direct short stock 的 borrow friction。", "表达 parity/financing view。"],
    clientType: "专业 directional 或 relative-value client。",
    suitability: "必须能承担 synthetic short 的线性上行亏损。",
    whenDealerSells: "Dealer 会把 short combo 还原成 synthetic short forward。",
    dealerExposure: "Short forward delta、short call assignment、dividend/borrow basis 是核心。",
    hedging: ["按 net forward delta hedge。", "检查 parity 和 borrow。", "管理 short call ex-dividend assignment。"],
    profitSource: "Execution spread、borrow/carry basis 和 parity edge。",
    commonMistakes: ["错误表达: Short Combo 是 protected short。正确说法是它近似 synthetic short forward。", "错误表达: long put 限制全部风险。正确说法是 short call 使 upside loss 仍接近线性。", "错误表达: options 结构绕开 borrow 问题。正确说法是 borrow 仍反映在 parity 中。"],
    tradeoffAnswer: "Short Combo 用 option legs 合成 short delta，trade-off 是保留 forward-like risk 并增加 assignment/carry 细节。",
    dealerAnswer: "Dealer 会从 put-call parity 角度看它，重点是 forward basis 和 short call events。",
    mistakeAnswer: "最常见错误是以为 long put 让风险有限。专业表达要说明 short call 复制了 short forward 的上行风险。",
  }),

  "short-guts": phase7aMemo({
    name: "Short Guts",
    directional: "近似中性 short-vol，但 ITM call/put 让 assignment 和 intrinsic value 更突出。",
    volatility: "-Vega：卖出 ITM options，IV 上升不利。",
    time: "+Theta：收大额 premium，但换来高 short Gamma 和 assignment risk。",
    convexity: "-Gamma：大幅单边移动会快速扩大亏损。",
    makesMoneyFrom: "标的留在两个 ITM strikes 之间，extrinsic value 衰减，实际波动低于 implied。",
    losesMoneyFrom: "标的大幅上行或下行，short ITM legs 的 intrinsic exposure 扩大。",
    bestMarketCondition: "高 IV、预期极低 realized vol、能主动管理 assignment。",
    worstScenario: "单边趋势或 gap move，ITM short options 亏损和 assignment 同时发生。",
    whyClientDoes: ["追求高 premium income。", "认为标的会停在宽区间内。", "愿意主动管理 ITM assignment。"],
    clientType: "非常高级 short-vol client。",
    suitability: "不适合普通收租客户；margin、assignment、liquidity 和 tail loss 都很重。",
    whenDealerSells: "Dealer 面对 Short Guts flow 会重点提示 ITM options 的操作风险。",
    dealerExposure: "Short ITM call/put 的 Delta、Gamma、Vega、assignment 和 wide bid-ask 都重要。",
    hedging: ["实时监控 net delta。", "管理 early assignment 和 exercise。", "对趋势行情做 stop/roll plan。"],
    profitSource: "ITM option bid-ask、short-vol premium 和 complex execution spread。",
    commonMistakes: ["错误表达: premium 大所以 cushion 大。正确说法是 premium 大是因为 intrinsic 和风险都大。", "错误表达: short guts 是高级版 short strangle。正确说法是 ITM assignment 和 liquidity 更复杂。", "错误表达: 横盘就一定赚。正确说法是要看 extrinsic、carry 和 exercise timing。"],
    tradeoffAnswer: "Short Guts 用很高 premium 换高度复杂的 ITM short-vol risk，不是普通 income strategy。",
    dealerAnswer: "Dealer 会重点看 assignment、ITM liquidity、net delta 和 short Gamma。",
    mistakeAnswer: "最常见错误是被大 premium 吸引。专业表达应拆 intrinsic/extrinsic，并强调操作风险。",
  }),

  "double-diagonal": phase7aMemo({
    name: "Double Diagonal",
    directional: "近似中性到宽区间：同时使用 call/put diagonal 管理两侧区间。",
    volatility: "通常 +Vega：long back-month options、short front-month options。",
    time: "+Theta from front-month shorts，但 long back-month legs 有 carry cost。",
    convexity: "Term-structure Gamma：near-term short Gamma 与 back-month long optionality 共存。",
    makesMoneyFrom: "前月 options 衰减、标的留在区间内、back-month IV 保持或上升。",
    losesMoneyFrom: "标的大幅突破区间，front short options 受压；term structure 或 IV crush 不利。",
    bestMarketCondition: "短期区间、长期 IV 支撑、term structure 有利。",
    worstScenario: "快速单边突破加前月 IV 上升/后月 IV 下跌，双侧管理困难。",
    whyClientDoes: ["用 calendar/diagonal 表达区间和 term structure view。", "希望比 iron condor 更有 long vega。", "利用不同 expiries 的 decay 差异。"],
    clientType: "理解 term structure 和 rolling 的 volatility client。",
    suitability: "需要能管理 expiry mismatch、short front assignment 和 rolling execution。",
    whenDealerSells: "Dealer 会把 Double Diagonal 看成 two-sided calendar/diagonal book。",
    dealerExposure: "Expiry-by-expiry Greeks、front Gamma、back Vega 和 skew term structure 是核心。",
    hedging: ["分 expiry 管理 Greeks。", "front-month short strikes 接近时提前 roll。", "监控 term structure shift。"],
    profitSource: "Calendar spread、term structure relative value 和多腿 execution spread。",
    commonMistakes: ["错误表达: Double Diagonal 是 iron condor 加时间。正确说法是它有 expiry mismatch 和 term Vega。", "错误表达: front theta 全是收入。正确说法是 short front Gamma 是代价。", "错误表达: 两边都有 long options 所以安全。正确说法是 short front legs 仍需管理。"],
    tradeoffAnswer: "Double Diagonal 用 expiry mismatch 换 theta 和 long vega exposure，trade-off 是 rolling 和 term-structure risk。",
    dealerAnswer: "Dealer 会分 expiry 看 Gamma/Vega，不会只看总 Greeks。",
    mistakeAnswer: "最常见错误是把它当 iron condor。专业表达要加入 term structure、front Gamma 和 roll plan。",
  }),

  "vega-套利": phase7aMemo({
    name: "Vega 套利",
    directional: "目标是尽量降低 Delta，核心暴露在 Vega/term structure，而不是 spot 方向。",
    volatility: "核心是 long/short Vega：比较不同 expiry、strike 或 surface bucket 的 implied vol。",
    time: "Theta 是 carry cost/收入，必须和 Vega edge 一起看。",
    convexity: "Gamma 取决于所用 options；不能假设 Vega trade 自动 delta-neutral 或 gamma-neutral。",
    makesMoneyFrom: "term structure 或 surface relative value 收敛，例如 cheap deferred vol vs rich front vol。",
    losesMoneyFrom: "vol spread 继续扩大、realized path 造成 hedge loss、theta carry 超过 edge。",
    bestMarketCondition: "有清晰 IV/RV、calendar 或 surface dislocation，并能控制 Delta/Gamma。",
    worstScenario: "事件冲击让 term structure 重新定价，相关 buckets 同时朝不利方向移动。",
    whyClientDoes: ["表达 volatility relative value。", "从方向交易升级到 surface/term structure 交易。", "训练 book-level Vega 思维。"],
    clientType: "Volatility trader、market-maker trainee 或专业学习账户。",
    suitability: "必须理解这是教学框架，不是无风险套利；需要 hedge cost、liquidity 和 model assumptions。",
    whenDealerSells: "Dealer 会把 Vega 套利拆成 expiry bucket、strike bucket 和 hedge cost。",
    dealerExposure: "term structure Vega、skew Vega、Gamma scalping cost 和 liquidity basis。",
    hedging: ["先 neutralize Delta，再看 bucketed Vega。", "对 parallel shift、twist、skew change 做 stress。", "记录 hedge P&L 和 carry。"],
    profitSource: "Vol surface mispricing、bid-ask、inventory warehousing 和 relative-value convergence。",
    commonMistakes: ["错误表达: Vega 套利是无风险套利。正确说法是 relative value trade，存在 model 和 execution risk。", "错误表达: Delta neutral 就没有方向风险。正确说法是 Gamma path 会重新产生 Delta。", "错误表达: 只看 total Vega。正确说法是要看 expiry/strike bucket。"],
    tradeoffAnswer: "Vega 套利的 trade-off 是用 hedge cost 和 model risk 换取 surface/term structure edge。",
    dealerAnswer: "Dealer 会按 term structure buckets 管理，而不是只说 long vega 或 short vega。",
    mistakeAnswer: "最常见错误是叫 arbitrage。专业表达要说 relative value、hedge cost、liquidity 和 model risk。",
  }),

  "delta-neutraldelta-中性": phase7aMemo({
    name: "Delta Neutral",
    directional: "目标是当前 Delta 接近 0，但 Gamma 会让 Delta 随 spot 变化。",
    volatility: "取决于结构；delta-neutral 可以是 long vol，也可以是 short vol。",
    time: "Theta 与 Gamma/Vega trade-off 绑定，不能单独看。",
    convexity: "Gamma 是核心：决定 rehedging 方向和 realized vol exposure。",
    makesMoneyFrom: "取决于结构：long gamma 赚 realized movement，short gamma 赚 theta/vol risk premium。",
    losesMoneyFrom: "hedge 频率、交易成本、gap move 或 vol 方向判断错误。",
    bestMarketCondition: "有明确 realized vs implied view，并能执行动态 hedge。",
    worstScenario: "Delta 看似中性但遇到 jump/gap，无法及时 rebalance。",
    whyClientDoes: ["学习把方向风险和波动率风险分开。", "表达 volatility view。", "管理 portfolio Delta limits。"],
    clientType: "专业训练、market-making 或 portfolio risk client。",
    suitability: "必须理解 delta-neutral 是状态，不是永久属性，也不是低风险保证。",
    whenDealerSells: "Dealer 会持续 rebalance Delta，并把 P&L 分解成 Gamma、Theta、Vega 和 costs。",
    dealerExposure: "Dynamic Delta、Gamma path、hedge slippage、Vega 和 liquidity。",
    hedging: ["设定 rebalance threshold。", "记录 hedge trades 和 realized vol。", "对 gap scenario 做 stress。"],
    profitSource: "Long gamma 的 realized vol edge 或 short gamma 的 theta/vol premium。",
    commonMistakes: ["错误表达: Delta neutral 就没有方向风险。正确说法是 Gamma 会让 Delta 重新出现。", "错误表达: 中性仓位不用管理。正确说法是需要 rebalance rule。", "错误表达: 只要 gamma scalping 就赚钱。正确说法是 realized vol 必须覆盖 theta 和成本。"],
    tradeoffAnswer: "Delta-neutral 的核心 trade-off 是 hedge cost、Gamma exposure 和 volatility view。",
    dealerAnswer: "Dealer 会把它看成动态过程，而不是一次性把 Delta 调到 0。",
    mistakeAnswer: "最常见错误是把 delta-neutral 当低风险。专业表达要强调 path、rebalance 和 gap。",
  }),

  "covered-put": phase7aMemo({
    name: "Covered Put",
    directional: "-Delta from short stock，short put 收 premium 但下跌收益被封顶。",
    volatility: "-Vega：卖出 put，IV 上升不利。",
    time: "+Theta：short put premium 随时间衰减。",
    convexity: "-Gamma：下跌接近 put strike 时风险和 assignment 变化加快。",
    makesMoneyFrom: "标的温和下跌或横盘，short stock 获利/put premium 衰减。",
    losesMoneyFrom: "标的大涨时 short stock 亏损；大跌时 short put 会迫使按 strike 买回股票，封住进一步收益。",
    bestMarketCondition: "温和看跌、愿意在目标价回补 short stock。",
    worstScenario: "标的大幅上涨，short stock 线性亏损且 put premium 无法补偿。",
    whyClientDoes: ["在 short stock 上收 premium。", "愿意在下方目标价 buy-to-cover。", "认为下跌空间有限。"],
    clientType: "有 short stock 能力和 borrow 管理经验的客户。",
    suitability: "需要理解 short stock 上行风险、borrow、dividend 和 put assignment。",
    whenDealerSells: "Dealer 会把 Covered Put 看成 covered call 的 bearish mirror，但 borrow/call-away 机制不同。",
    dealerExposure: "Short stock borrow、short put assignment、downside cap 和 upside short-stock risk。",
    hedging: ["监控 borrow 和 dividend。", "对 upside squeeze 做 stress。", "管理 short put assignment 与 stock cover。"],
    profitSource: "Put premium execution、borrow/carry assumptions 和 stock/option execution spread。",
    commonMistakes: ["错误表达: Covered Put 是低风险 bearish income。正确说法是 short stock 上行风险仍很大。", "错误表达: covered 表示完全保护。正确说法是 put 只覆盖下方回补机制。", "错误表达: 和 covered call 完全镜像。正确说法是 borrow 和 dividend 让它不同。"],
    tradeoffAnswer: "Covered Put 用 short put premium 增强 short stock，但牺牲进一步下跌收益并保留上行风险。",
    dealerAnswer: "Dealer 会关注 borrow、dividend、assignment 和 short squeeze。",
    mistakeAnswer: "最常见错误是忽略 short stock 上行风险。专业表达要把 borrow 和 squeeze 放进去。",
  }),

  "stock-repair-covered-ratio-spread": phase7aMemo({
    name: "Stock Repair / Covered Ratio Spread",
    directional: "+Delta from stock，ratio call spread 试图在温和反弹中加速修复。",
    volatility: "通常 short upside Vega，因为卖出更多 calls 融资 long call。",
    time: "Theta 取决于 ratio legs；反弹不足时结构可能失效。",
    convexity: "温和上行有利，强烈上行会被额外 short calls 封顶或反转。",
    makesMoneyFrom: "标的从亏损区反弹到 short call 区域，ratio spread 加速回本。",
    losesMoneyFrom: "标的继续下跌，股票亏损继续；暴涨时 upside 被 capped 或出现 short call 风险。",
    bestMarketCondition: "持股被套后预期温和反弹，但不期待暴涨。",
    worstScenario: "继续大跌，repair 结构无法保护本金；或暴涨穿过 short calls 造成机会成本/风险。",
    whyClientDoes: ["想降低 break-even。", "不想投入太多新 premium。", "愿意用上方收益换修复速度。"],
    clientType: "持股被套但仍愿意持有的 stock client。",
    suitability: "必须披露它不是 downside hedge，也不是保证回本。",
    whenDealerSells: "Dealer 会把 stock repair 拆成 stock plus ratio call spread。",
    dealerExposure: "Stock Delta、short call ratio tail、assignment 和 upside cap。",
    hedging: ["展示修复后的 break-even。", "对继续下跌 scenario 做 stress。", "检查 short calls 是否需要加 wing。"],
    profitSource: "Ratio spread execution、call skew 和客户定制 repair payoff 的 spread。",
    commonMistakes: ["错误表达: Stock Repair 能无成本回本。正确说法是用上方 payoff concession 换较低 break-even。", "错误表达: 它保护继续下跌。正确说法是主要 downside 仍来自 stock。", "错误表达: 反弹越多越好。正确说法是超过 short calls 后 upside 被限制。"],
    tradeoffAnswer: "它用上方收益让渡换取较低修复 break-even，不是免费保险。",
    dealerAnswer: "Dealer 会看 stock Delta 和 short call ratio risk，并确认客户理解 upside cap。",
    mistakeAnswer: "最常见错误是说它免费修复。专业表达要说清 payoff concession。",
  }),

  "double-bull-spread": phase7aMemo({
    name: "Double Bull Spread",
    directional: "+Delta：通常叠加 call/put bull spreads 表达更强 bullish view。",
    volatility: "Vega mixed：不同 legs 可能部分抵消，但 skew 会影响 entry。",
    time: "Theta 取决于 debit/credit mix 和 spot 区间。",
    convexity: "Defined-risk directional convexity，收益区间比单一 spread 更复杂。",
    makesMoneyFrom: "标的上涨进入 bullish target zone，多个 spread 同时贡献收益。",
    losesMoneyFrom: "标的不涨或下跌，debit/credit legs 共同拖累。",
    bestMarketCondition: "明确看涨、希望用 defined-risk 多结构提高 target-zone exposure。",
    worstScenario: "标的下跌或 IV/skew 不利导致两个 spreads 同时恶化。",
    whyClientDoes: ["想比单一 bull spread 更强表达看涨。", "希望 max loss 有定义。", "利用 calls 与 puts 的相对定价。"],
    clientType: "能理解 multi-spread payoff 的 directional client。",
    suitability: "需要说明 legs 多、execution cost 高、payoff 不如单一 vertical 直观。",
    whenDealerSells: "Dealer 会拆成两个 bullish spreads 管理，而不是看一个总标签。",
    dealerExposure: "多个 strikes 的 Delta/Gamma、skew 和 execution liquidity。",
    hedging: ["逐 spread 拆 Greeks。", "检查 combined max loss。", "关注 execution slippage。"],
    profitSource: "Multi-leg bid-ask、skew differential 和定制 bullish payoff 的 spread。",
    commonMistakes: ["错误表达: Double Bull 一定比单一 bull spread 更好。正确说法是成本和复杂度也更高。", "错误表达: defined risk 就能加倍仓位。正确说法是按组合 max loss sizing。", "错误表达: 两个 spread 自动分散。正确说法是方向风险高度相关。"],
    tradeoffAnswer: "Double Bull Spread 增强 bullish exposure，但用更高复杂度和 execution cost 换取。",
    dealerAnswer: "Dealer 会拆 legs、看 combined Greeks 和 skew，而不是按名称报价。",
    mistakeAnswer: "最常见错误是把 double 当 diversification。专业表达要讲 correlated directional risk。",
  }),

  "double-bear-spread": phase7aMemo({
    name: "Double Bear Spread",
    directional: "-Delta：叠加 bearish spreads 表达更强 downside view。",
    volatility: "Vega mixed，但 put skew 通常显著影响成本和 edge。",
    time: "Theta 取决于 debit/credit 组合和 spot 位置。",
    convexity: "Defined-risk bearish convexity，收益集中在下方 target zone。",
    makesMoneyFrom: "标的下跌进入目标区间，多个 bearish spreads 同时贡献收益。",
    losesMoneyFrom: "标的不跌、反弹或 put skew 回落不利。",
    bestMarketCondition: "明确看跌但希望 max loss 有定义。",
    worstScenario: "标的上涨或横盘，多个 bearish legs 同时消耗。",
    whyClientDoes: ["想比单一 bear spread 更强表达看跌。", "保持 defined-risk。", "利用不同 strikes 的 put/call relative value。"],
    clientType: "有明确 downside target 的 directional client。",
    suitability: "需要能解释 cost、max loss、target zone 和多腿流动性。",
    whenDealerSells: "Dealer 会拆成两个 bearish spreads 管理 Greeks 和 skew。",
    dealerExposure: "Downside Delta/Gamma、put skew、wing liquidity 和 execution cost。",
    hedging: ["对 gap-down 和 rebound 都做 scenario。", "检查 combined max loss。", "按 strike bucket 管理 Greeks。"],
    profitSource: "Multi-leg bid-ask、skew differential 和 custom bearish payoff spread。",
    commonMistakes: ["错误表达: Double Bear 是更安全的看跌。正确说法是只是 defined-risk，不代表高胜率。", "错误表达: 结构多就更分散。正确说法是方向风险集中。", "错误表达: 只看到期 payoff。正确说法是 put skew 和 liquidity 会影响中途管理。"],
    tradeoffAnswer: "Double Bear Spread 增强 downside target exposure，但带来更高 execution cost 和更复杂管理。",
    dealerAnswer: "Dealer 会关注 put skew、strike buckets 和 combined max loss。",
    mistakeAnswer: "最常见错误是把复杂度当分散。专业表达要说清 correlated bearish exposure。",
  }),

  "short-call-calendar-spread": phase7aMemo({
    name: "Short Call Calendar Spread",
    directional: "通常偏离 strike 的 directional/term view：short back-month call、long front-month call 或相反定义需先确认。",
    volatility: "-Vega term exposure：short longer-dated vol 是核心风险。",
    time: "Theta 可能为正，但取决于 front/back premium 和 spot 路径。",
    convexity: "Expiry mismatch Gamma：near-term leg 会快速变化。",
    makesMoneyFrom: "后月 IV 回落、term structure 收敛，或标的远离 strike 使 calendar value 下降。",
    losesMoneyFrom: "后月 IV 上升、标的停在 strike 附近、front leg 到期后留下 naked/less-hedged back risk。",
    bestMarketCondition: "认为后月 call vol 太贵，且有清晰的 roll/close plan。",
    worstScenario: "front leg 到期失去保护后，back-month short call 暴露于 upside move。",
    whyClientDoes: ["表达 short term-structure Vega。", "认为远月 call overpriced。", "用 calendar 结构做 relative value。"],
    clientType: "高级 vol/term-structure client。",
    suitability: "必须理解 short calendar 不是普通 calendar；远月 short option 风险更重。",
    whenDealerSells: "Dealer 会先确认 calendar 方向，并按 expiry Vega/Gamma 管理。",
    dealerExposure: "Short back Vega、front Gamma、roll risk 和 short call assignment。",
    hedging: ["分 expiry 看 Greeks。", "front expiry 前制定 close/roll。", "对 upside gap 和 IV up 做 stress。"],
    profitSource: "Term structure spread、calendar bid-ask 和 carry/vol relative value。",
    commonMistakes: ["错误表达: Calendar 都是 long Vega。正确说法是 short calendar 通常 short longer-dated Vega。", "错误表达: front leg 保护到最后。正确说法是 expiry mismatch 会留下残余风险。", "错误表达: 只看 theta。正确说法是 term Vega 和 roll risk 更关键。"],
    tradeoffAnswer: "Short Call Calendar 用 short longer-dated vol 换 carry/relative value，trade-off 是 term Vega 和 roll risk。",
    dealerAnswer: "Dealer 会按 expiry bucket 管理，而不是用一个 net Vega 概括。",
    mistakeAnswer: "最常见错误是把它和 long calendar 混淆。专业表达必须先确认 long/short 哪个 expiry。",
  }),

  "short-put-calendar-spread": phase7aMemo({
    name: "Short Put Calendar Spread",
    directional: "通常围绕 downside strike 的 term-structure short-vol view。",
    volatility: "-Vega term exposure，尤其对后月 put skew/vol 上升敏感。",
    time: "Theta 可能为正，但 downside move 会让 front/back legs 非线性变化。",
    convexity: "Expiry mismatch Gamma；front put 到期后可能留下后月 short put 风险。",
    makesMoneyFrom: "后月 put vol 回落、term structure 收敛，或标的远离 strike。",
    losesMoneyFrom: "downside move、put IV spike、front protection 到期后 short back put 暴露。",
    bestMarketCondition: "认为远月 put vol/skew 过贵，且 downside gap 风险可控。",
    worstScenario: "市场下跌且后月 put IV 飙升，short back-month put 成为核心风险。",
    whyClientDoes: ["表达 short downside term Vega。", "利用 put skew/term structure relative value。", "构建教育型 vol carry trade。"],
    clientType: "高级 vol client 或专业训练账户。",
    suitability: "必须能承担 short put assignment、margin 和 term-structure risk。",
    whenDealerSells: "Dealer 会将 Short Put Calendar 拆成 front expiry Gamma 与 back expiry Vega。",
    dealerExposure: "Short back put Vega/skew、front leg expiry risk、assignment 和 liquidity。",
    hedging: ["对 gap-down + IV up 做 stress。", "front expiry 前主动 roll/close。", "按 expiry bucket 管理 Vega。"],
    profitSource: "Put term structure spread、skew premium 和 calendar execution spread。",
    commonMistakes: ["错误表达: Short Put Calendar 是低风险收 theta。正确说法是 short back put Vega/skew 风险很大。", "错误表达: front put 永远保护。正确说法是 front expiry 后保护消失。", "错误表达: Put calendar 和 call calendar 一样。正确说法是 put skew 和 assignment 更重要。"],
    tradeoffAnswer: "Short Put Calendar 用 short longer-dated downside vol 换 carry，trade-off 是 gap、skew 和 expiry mismatch。",
    dealerAnswer: "Dealer 会看 back-month put Vega 和 front expiry roll，而不是只看 net premium。",
    mistakeAnswer: "最常见错误是只看 theta。专业表达应突出 short term Vega、put skew 和 roll risk。",
  }),
